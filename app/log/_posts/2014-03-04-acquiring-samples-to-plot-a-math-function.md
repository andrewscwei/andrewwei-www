...
5.  Evaluate $$f(x)$$ at all 5 $x$-intervals: $$x_a$$, $$x_{ab}$$, $$x_b$$, $$x_{bc}$$, and $$x_c$$, and store these points in an array.

6.  If `depth <= 0`, return the array as is.

7.  Otherwise walk through each point in the array to determine if there are rapid oscillations. Use an integer `count` defaulted to 0 to measure the result: either there exists rapid oscillations and requires further refinements or there aren’t any. Here are the steps to determine:
